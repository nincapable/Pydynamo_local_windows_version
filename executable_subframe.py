...
class App(tk.Tk):
    ...
    def add_subframe(self):
        ...
        code_text = scrolledtext.ScrolledText(new_subframe, wrap=tk.WORD, height=5, width=40)
        code_text.pack(fill=tk.BOTH, expand=True)

        execute_button = tk.Button(new_subframe, text="Exécuter", command=lambda: self.execute_code(code_text))
        execute_button.pack(pady=5)

        # Widget de texte pour afficher la sortie
        output_text = scrolledtext.ScrolledText(new_subframe, wrap=tk.WORD, height=5, width=40)
    ...
    def execute_code(self, code_text):
        ...
        output_text = self.subframes[-1][1]
